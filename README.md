...
Figure 6. Expression trajectories of five biologically selected genes across velum development stages (Early, Thin, Mature). Lines connect stage means; shaded ribbons: +/-1 SE; individual replicate points overlaid (jittered). Genes: FLO11 (cell adhesion, YHR211W), CYC1 (iso-1 cytochrome c, YJR048W), QCR2 (Complex III subunit, YPR191W), ALD4 (mitochondrial aldehyde dehydrogenase, YOR374W), ALD6 (cytosolic aldehyde dehydrogenase, YPL061W). VST-normalised counts.

To connect transcriptome-wide patterns to specific, functionally characterised genes, VST-normalised expression trajectories were plotted for five genes selected on the basis of established roles in velum physiology and wine chemistry (Figure 6; see Section 2.7.3 for selection rationale). FLO11 showed progressive induction across all three stages, consistent with its reported dominance of flocculin transcripts in mature flor velum (Mardanov et al., 2020) and its essential role in cell-cell adhesion (Bruckner & Mosch, 2012). CYC1 and QCR2, encoding components of the mitochondrial electron transport chain, both showed their largest expression gain between the Thin and Mature stages, directly mirroring the specific enrichment of electron transport chain terms in the Mature vs Thin GO panel (Figure 5). ALD4 and ALD6, encoding mitochondrial and cytosolic aldehyde dehydrogenase isoforms respectively, showed progressive induction through velum maturation, providing a direct transcriptomic link to the acetaldehyde accumulation that is the chemical hallmark of biologically-aged sherry wines (Lasanta et al., 2003).
...
The GO enrichment data (Figure 5) corroborate these gene-level findings at the pathway level. Transmembrane transport was the top-ranked term in all three contrasts, with the highest GeneRatio and most significant enrichment in the Thin vs Early contrast (GeneRatio ~0.15, p.adjust ~1x10-15), indicating that remodelling of membrane transport is an early and dominant feature of velum initiation. The Mature vs Thin panel was most specifically enriched for mitochondrial electron transport chain and oxidative phosphorylation terms, demonstrating that the respiratory transition is completed primarily between the Thin and Mature stages. The proteomic data of Moreno-Garcia et al. (2017) independently confirmed elevated abundance of respiratory chain components in the biofilm state, providing multi-omics corroboration.


## **4.2 Implications**

The progressive accumulation of DEGs across the developmental axis—with each transition adding hundreds of newly regulated genes—implies that velum maturation requires sustained transcriptional input rather than a single committed developmental switch. This has direct relevance to the biological ageing of sherry-style wines: the accumulation of acetaldehyde and acetals, the principal chemical signatures of biological ageing, is driven by oxidative ethanol metabolism that is only fully active in the mature velum (Mardanov et al., 2020). Understanding the transcriptional determinants of this transition could inform selection of flor strains with superior oxidative capacity or provide molecular targets for optimising velum establishment in commercial wine production.

The strong enrichment of transmembrane transport processes across all three contrasts (Figure 5) also has applied significance. Flor yeast cells in mature velum must acquire nutrients from a carbon-limited medium while exporting ethanol oxidation products. Induction of high-affinity transport systems is an anticipated adaptive response, and the specific transporters up-regulated here could serve as biomarkers of velum maturity or as engineering targets for strains with improved survival under harsh wine conditions.

## **4.3 Limitations**
Bulk RNA-seq of whole-velum tissue integrates transcriptomes from cells at distinct microenvironmental positions within the biofilm, obscuring cell-type-specific expression programmes (Mardanov et al., 2020). With n = 3 replicates per stage, statistical power for detecting genes with moderate fold changes is reduced and DESeq2 dispersion estimates may be imprecise for low-count genes (Love et al., 2014). Although ORA was completed for all three contrasts, GSEA was not executed in the current work, meaning coordinated shifts in moderately regulated genes below the |LFC| > 1 threshold may be underrepresented. Finally, the expression trajectories in Figure 6 are descriptive; causal roles for the selected genes in velum formation require experimental validation.

# **5.0 Version Information**
All analyses were conducted in R version 4.3.x. Key package versions: DESeq2 v1.42, tximport v1.28, txdbmaker v1.0, clusterProfiler v4.10, org.Sc.sgd.db v3.18, ggplot2 v3.4, patchwork v1.2, pheatmap v1.0.12, ggrepel v0.9, ashr v2.2. Read quantification: Salmon v1.10 (selective-alignment, decoy-aware). QC: FastQC v0.12, MultiQC v1.14. Reference: S. cerevisiae Ensembl R64-1-1 release 110. Full version information reproducible via sessionInfo().
